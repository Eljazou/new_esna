...
<div class="box">
    ...
    <div class="box-body">
        <table id="example1" class="table table-bordered table-striped">
            ...
            <?php foreach ($jourferiers as $jourferier): ?>
                <tr>
                    <td class="jour-name"><?php echo h($jourferier['Jourferier']['name']); ?>&nbsp;</td>
                    <td class="date-cell"><?php echo h($jourferier['Jourferier']['date_debut']); ?>&nbsp;</td>
                    <td class="date-cell"><?php echo h($jourferier['Jourferier']['date_fin']); ?>&nbsp;</td>
                    <td><?php
                        $now = strtotime($jourferier['Jourferier']['date_debut']);
                        $your_date = strtotime($jourferier['Jourferier']['date_fin']);
                        $datediff = $your_date - $now;
                        $j = floor($datediff / (60 * 60 * 24));
                        $j++;
                        $dureeClass = ($j < 0) ? 'duree-negative' : 'duree-positive';
                        echo '<span class="duree-badge '.$dureeClass.'">'.$j.' jours</span>';
                        ?>
                    </td>
                    <td class="actions">
                        <?php echo $this->Html->link(
                            ...
                            array('action' => 'edit', $jourferier['Jourferier']['id']),
                        ...
                        <?php echo $this->Form->postLink(
                            ...
                            array('action' => 'delete', $jourferier['Jourferier']['id']),
                            array('class'=>'btn-action btn-danger-outline', 'escape' => false),
                            null,
                            __('Etes-vous sur de vouloir supprimer # %s?', $jourferier['Jourferier']['id'])
                        ); ?>
                    </td>
                </tr>
            <?php endforeach; ?>
