                        ...
                        <tbody>
                            ...
                            foreach ($vmp as $visite) {
                                ...
                                for ($i = 1; $i < 8; $i++) {
                                    ...
                                    echo "<td data='$distance' style='font-weight:bold;$c'><b class='detailbtn' style='cursor:pointer;' id='$udid'><c id='" . $udid . "_m'> $vv_med</c> <br><c id='" . $udid . "_p'>$vv_pharm</c> </b></td>";
                                }
                                ...
                                $t_med = $t_med ?? 0;
                                $t_pharm =  $t_pharm ?? 0;
                                $c_med = "#34c180";    // Default color (green)
                                $c_pharm = "#34c180";  // Default color (green)

                                // Handle zero objectives
                                if ($objectifglobale == 0) {
                                    $t = 0;
                                }
                                if ($objectifmedcin > 0) {
                                    $t_med = round(($t_med / $objectifmedcin) * 100, 2);
                                }
                                if ($objectifpharma > 0) {
                                    $t_pharm = round(($t_pharm / $objectifpharma) * 100, 2);
                                }

                                // Assign colors based on percentage for med
                                if ($t_med < 50) {
                                    $c_med = "red";
                                } elseif ($t_med < 75) {
                                    $c_med = "#f5aa02"; // orange
                                ...
                                // Assign colors based on percentage for pharm
                                if ($t_pharm < 50) {
                                    $c_pharm = "red";
                                } elseif ($t_pharm < 75) {
                                    $c_pharm = "#f5aa02"; // orange
                                }

                                // Output results
                                echo "<td>
                                        <span style='color:$c_med; font-weight:bold;'>$t_med% </span><br>
                                        <span style='color:$c_pharm; font-weight:bold;'>$t_pharm%</span>
                                      </td>";
                                echo "</tr>";
                            }
                            ?>
...
<script>
    ...
    $(".detailbtn").click(function() {
        ...
        for (i; i < arr.length; i++) {
            ...
            var tr = "<tr class='tremove'><td>" + solo[id][date][key] + "<a target='_blank' href='/clients/view/" + arr[i][0] + "'>" + nom + "</a></td><td>" + activite + "</td><td>" + pot + "</td><td>" + pot1 + "</td><td>" + arr[i][1] + " " + arr[i][2] + "</td><td>" + visites_array[id][date][idc] + "</td></tr>";
